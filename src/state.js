...
function createChallenge({ channelId, creatorId, teams, tasks, timing, visibility = 'all', now = Date.now() }) {
...
function castVote(state, userId, choice, now = Date.now()) {
  ...
  if (now >= state.vote.expiresAt) {
    ...
    return { result: 'timeout_end', endVotes: countVotes(state).end, continueVotes: countVotes(state).continue };
  }
  ...
  if (counts.end >= majority) {
    ...
    return { result: 'end', endVotes: counts.end, continueVotes: counts.continue };
  }

  if (counts.continue >= majority) {
    state.vote.status = 'continued';
    return { result: 'continue', endVotes: counts.end, continueVotes: counts.continue };
  }

  return { result: 'pending', endVotes: counts.end, continueVotes: counts.continue };
}
...
function getWinnerTeam(state) {
  ...
  if (state.firstFinishTeamId) return state.teams.find((team) => team.id === state.firstFinishTeamId) || null;
  return null;
}
...
function summarizeTeam(state, team, winnerTotalMs = null) {
  ...
  const delta = winnerTotalMs !== null && totalMs !== null && team.finishedAt
