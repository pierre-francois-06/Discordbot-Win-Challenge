...
function buildTeamUserPrompt(sessionId, teamIndex, teamCount) {
  return {
    content: `Setup Schritt 2: Waehle die Discord-User fuer Team ${teamIndex + 1}/${teamCount}.`,
    embeds: [],
    components: [
      new ActionRowBuilder().addComponents(
        new UserSelectMenuBuilder()
          .setCustomId(`wc:setup:team:${sessionId}:${teamIndex}`)
          .setPlaceholder(`User fuer Team ${teamIndex + 1}`)
          .setMinValues(1)
          .setMaxValues(25)
      )
    ],
...
function buildVisibilityPrompt(sessionId) {
  return {
    content: 'Setup Schritt 3: Was duerfen Teams waehrend der Challenge oeffentlich sehen?',
    embeds: [],
    components: [
      new ActionRowBuilder().addComponents(
        new ButtonBuilder()
...
function buildTaskReviewPrompt(sessionId, tasks) {
  ...
  return {
    content: 'Setup Schritt 4: Fuege Aufgaben einzeln hinzu.',
    embeds: [embed],
...
function buildB2bPrompt(sessionId, pendingTask) {
  const embed = new EmbedBuilder()
    .setTitle('BxB fuer diese Aufgabe?')
...
function buildChallengeStatus(state) {
  ...
  if (state.firstFinishTeamId && state.vote?.status === 'open') {
    ...
    lines.push(`${firstTeam?.name || 'Ein Team'} ist fertig. Abstimmung laeuft bis <t:${Math.floor(state.vote.expiresAt / 1000)}:T>.`);
  }
...
function buildMyTasksMenu(state, teamId, publicMessageId) {
  ...
  return {
    content: `${team.name}: Welche Aufgabe moechtest du abhaken?`,
    embeds: [],
    components: [
      new ActionRowBuilder().addComponents(
        new StringSelectMenuBuilder()
          .setCustomId(`wc:complete:${publicMessageId}:${teamId}`)
          .setPlaceholder('Eine erledigte Aufgabe waehlen')
          .setMinValues(1)
          .setMaxValues(1)
