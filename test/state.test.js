...
test('castVote uses majority and rejects spectators', () => {
  ...
  assert.throws(() => castVote(state, 'spectator', 'end', 122000), /Mitspieler/);
  assert.equal(castVote(state, 'u1', 'end', 122000).result, 'pending');
  assert.equal(castVote(state, 'u2', 'end', 123000).result, 'end');
});
